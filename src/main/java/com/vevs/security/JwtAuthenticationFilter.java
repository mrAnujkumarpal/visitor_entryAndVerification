...
import com.vevs.common.exception.JwtTokenMissingException;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Autowired
    private com.vevs.security.AppUserDetailsService customUserDetailsService;
    ...
    private String getJwtFromRequest(HttpServletRequest request) throws JwtTokenMissingException {
  ...
  /*  private String getJwtFromRequest(HttpServletRequest request) throws JwtTokenMissingException {
        logger.info("inside JWT AUthentication filter of getJwtFromRequest");
        String bearerToken = request.getHeader("Authorization");
        if (bearerToken == null || !bearerToken.startsWith("Bearer ")) {
            //  throw new JwtTokenMissingException("JWT token is missing from header");
           // new HttpResponse().errorResponse("Login Token missing from the header");
          //  return null;
        }
        if (StringUtils.hasText(bearerToken) && bearerToken.startsWith("Bearer ")) {
            return bearerToken.substring(7);
